...
extractGeneCoords=' BEGIN { start=300000000; end=0 } { chr= $1; if ($2<start) start=$2; if ($3>end) end=$3 } END { print chr, start, end }'
geneArgs=`grep -w $gene $refGeneFile | cut -f 3,5,6 | grep -v _ | awk "$extractGeneCoords"`

# need to avoid funny chromosomes which may have an underscore in them
